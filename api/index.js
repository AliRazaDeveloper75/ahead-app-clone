...
const BASE_DIR = isProduction ? '/tmp' : path.join(__dirname, '..', 'server');
const DATA_FILE = path.join(BASE_DIR, 'data.json');
const UPLOADS_DIR = path.join(BASE_DIR, 'uploads');

// Ensure directories and data file exist in a way that doesn't crash on read-only FS
try {
    ...
    console.warn('Filesystem initialization warning:', error.message);
}

// Serve static files from both local and tmp uploads
...
const getUsers = () => {
    try {
        return fs.readJsonSync(DATA_FILE).users || [];
    } catch (e) {
        return [];
    }
};
...
app.post('/api/payment/manual', upload.single('screenshot'), (req, res) => {
    ...
    if (user) {
        ...
        user.paymentProof = req.file ? req.file.filename : null;
